...
lcd.print("Zuniguns");// you can add your team name or someting cool
  lcd.setCursor(0,1);
lcd.print("Bomba V1.0");// you can add your team name or someting cool
...
void domination(){
  ...
  while(1)  // this is the important code, is a little messy but works good.
  {
    ...
    if(showGameTime){ //THE SECOND IS /2
      ...
      if(team == 1)lcd.print("---TIME A---");
      if(team == 2)lcd.print("---TIME B---");
...
void gameOver(){
  ...
  while(!defusing){
    ...
    lcd.clear();
    lcd.setCursor(3,0);
    lcd.print("PISTA FRIA!");
    lcd.setCursor(0,1);

    //check who team win the base
    if(greenTime>redTime){
      //greenteam wins
      lcd.print(" TIME A VENCEU!");
      digitalWrite(GREENLED, HIGH);
    }
    else{
      //redteam wins 
      lcd.print(" TIME B VENCEU!");
      digitalWrite(REDLED, HIGH);
    }
    delay(3000);
    keypad.getKey();
